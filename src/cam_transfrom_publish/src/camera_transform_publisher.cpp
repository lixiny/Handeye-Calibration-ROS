...
public:
    ...
    ChessBoardExtractor(ros::NodeHandle& nh_, cv::Size boardSize_, float squareSize_, cv::Mat cameraMatrix_, cv::Mat distCoeffs_ )
    ...
    void imageCb_rgb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        bool chessBoardFound = cv::findChessboardCorners(rgb, boardSize,  pointBuf, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_NORMALIZE_IMAGE);
        if( ! chessBoardFound) {
            // TODO do someting
            cv::imshow("image", rgb);
            cv::waitKey(1);
...
int main(int argc, char** argv) 
{
    ...
    ros::NodeHandle nh2;
    ChessBoardExtractor ce( nh2, boardSize, squareSize, cameraMatrix, distCoeffs);
