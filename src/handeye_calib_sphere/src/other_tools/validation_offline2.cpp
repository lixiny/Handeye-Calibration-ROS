...
//    double x[10] = {0.706926, -0.705006, -0.0521853, -0.0223529, -0.035389, 0.033641, -0.0678332};  //depth
    double x[10] = {0.706926, -0.736006, -0.0500853, -0.0243229, -0.032538, 0.0329437, -0.064823};  //color
    Eigen::Quaterniond q;
    q.x() = x[0];
    q.y() = x[1];
    q.z() = x[2];
    ...
    Eigen::Matrix4d X = T.inverse().matrix();
    ...
    // string tbestsfile = "/home/sirius/rgbd_ws/src/sphere_handeye/dataset/tbest_depthcalb.xml";
    string tbestsfile = "/home/sirius/rgbd_ws/src/sphere_handeye/dataset/tbest_colorcalb.json";
    ...
    for (int k = 0; k < numOfFiles; k++)
    {
        ...
        Eigen::Vector4d tj(tj_icp(0), tj_icp(1), tj_icp(2), 1.0);

        Eigen::Vector4d tbest = tfTransformMatrix * X * tj;
        Eigen::Vector3d tb_est(tbest(0), tbest(1), tbest(2));
        cout << "tbest esitmate is :" << tbest.transpose() << endl;
        ...
        if (abs(radius - 0.075) > 0.003)
        {
            continue;
        } else {
            // cout << "tj icp is :" << tj_icp.transpose() << endl;
            tbests.push_back(tb_est);
            error += calculateDistance(tb_est, tb);
            counter ++;
    ...
    cv::Mat TBsMat(counter, 3, CV_64F);
    for (int g = 0; g < counter; g++){
        TBsMat.at<double>(g,0) = tbests[g](0);
        TBsMat.at<double>(g,1) = tbests[g](1);
        TBsMat.at<double>(g,2) = tbests[g](2);
    }

    FileStorage fswrite(tbestsfile, FileStorage::WRITE);
    fswrite << "tbest_calb" << TBsMat;
    fswrite.release();
